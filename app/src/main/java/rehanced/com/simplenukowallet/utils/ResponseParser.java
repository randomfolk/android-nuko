...
import rehanced.com.simplenukowallet.data.TokenDisplay;
import rehanced.com.simplenukowallet.data.TransactionDisplay;
import rehanced.com.simplenukowallet.data.WalletDisplay;
...
public class ResponseParser {
    ...
    public static ArrayList<TransactionDisplay> parseTransactions(String response, String walletname, String address, byte type, boolean Findall) {
        Log.d("parseTx", "tesT  " + response);
        JSONArray data;
        ArrayList<TransactionDisplay> erg = new ArrayList<TransactionDisplay>();
        try {
            data  = new JSONObject(response).getJSONArray("result");
            for (int i = 0; i < data.length(); i++) {
                if (TxCache.getInstance().contains(data.getJSONObject(i).getString("hash"))) {
                    Log.d("TxCache", "contains " + data.getJSONObject(i).getString("hash"));
                }
                TxCache.getInstance().put(data.getJSONObject(i).getString("hash"), data.getJSONObject(i).toString());
            }
        }catch (Exception e){
            Log.d("ParseTx","exception 1st part " + e.getMessage());
        }
        try{
            data = new JSONArray();
            Set<String> tx_set = TxCache.getInstance().getAllKeys();
            for (String tx : tx_set) {
                Log.d("txCache","put" + TxCache.getInstance().get(tx));
                data.put(new JSONObject(TxCache.getInstance().get(tx)));
            };
            ...
            for (int i = 0; i < data.length(); i++) {
                ...
                String vorzeichen = "+";
                if(!Findall && (!address.equalsIgnoreCase(from) && !address.equalsIgnoreCase(to))){ continue;}
                ...
                if (remove0x(data.getJSONObject(i).getString("value")).equals("0") && !Settings.showTransactionsWithZero)
                    continue; // Skip contract calls or empty transactions

                String value_String = new BigInteger(remove0x(data.getJSONObject(i).getString("value")),16).toString();
                Log.d("ResponseParse",value_String);
                if (TxCache.getInstance().contains(data.getJSONObject(i).getString("hash"))){
                    Log.d("TxCache","contains "+ data.getJSONObject(i).getString("hash"));
                }
                TxCache.getInstance().put(data.getJSONObject(i).getString("hash"),data.getJSONObject(i).toString());
                ...
                erg.add(new TransactionDisplay(
                        from,
                        to,
                        new BigInteger(vorzeichen + value_String),
            ...
            Log.d("ParseTx","exception " + e.getMessage());
        ...
        for (int i = 0; i < storedwallets.size(); i++) {
            ...
            for (int j = 0; j < data.length(); j++) {
                if (data.getJSONObject(j).getString("account").equalsIgnoreCase(storedwallets.get(i).getPubKey())) {
                    balance = new BigInteger(remove0x(data.getJSONObject(j).getString("balance")),16);
                    break;
                }
            }
    ...
    public static String parseBalance(String response, int comma) throws JSONException {
        ...
        if (balance.equals("0") || balance.equals("0x0")) return "0";
        return new BigDecimal(new BigInteger(remove0x(balance),16)).divide(new BigDecimal(1000000000000000000d), comma, BigDecimal.ROUND_UP).toPlainString();
    }
