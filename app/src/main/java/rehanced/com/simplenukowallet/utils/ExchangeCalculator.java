...
import rehanced.com.simplenukowallet.data.CurrencyEntry;
...
public class ExchangeCalculator {
    ...
    private DecimalFormat formatterBTCExact = new DecimalFormat("###,###.########");
    ...
    public String displayBalanceNicely(double d) {
        if (index == 2)
            ...
            return displayEthNicelyExact(d);
        else
            return displayBTCNicelyExact(d);
    }
    ...
    public String displayBTCNicelyExact(double d) {
        return formatterBTCExact.format(d);
    }
    /**
    ...
    public String convertRateExact(BigDecimal balance, double rate) {
        double real_balance = balance.doubleValue() * rate;
        if (index == 2) {
            return displayUsdNicely(Math.floor(balance.doubleValue() * rate * 100) / 100) + "";
        } else if (real_balance >0.00001)
            return displayEthNicelyExact(balance.multiply(new BigDecimal(rate)).setScale(7, RoundingMode.CEILING).doubleValue());
        else
            return displayBTCNicelyExact(balance.multiply(new BigDecimal(rate)).setScale(10, RoundingMode.CEILING).doubleValue());
    }
        ...
        EtherscanAPI.getInstance().getEtherPrice(new Callback() {
            ...
            public void onResponse(Call call, final Response response) throws IOException {
                try {
                    ...
                    JSONObject data = new JSONArray(body).getJSONObject(0);;

                    Log.d("test",data.toString());
                    Log.d("rate",String.valueOf(data.getDouble("price_btc")));
                    conversionNames[1].setRate(data.getDouble("price_btc"));
                    conversionNames[2].setRate(data.getDouble("price_usd"));
